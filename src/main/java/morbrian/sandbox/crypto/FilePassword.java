...
/**
 * Read a password in opaque form to a file for use with the FilePassword accessor in conjunction
 * with the JaasSecurityDomain {CLASS}org.jboss.security.plugins.FilePassword:password-file format
 * of the KeyStorePass attribute. The original opaque password file can be created by running: java
 * org.jboss.security.plugins.FilePassword salt count password password-file Running java
 * org.jboss.security.plugins.FilePassword will generate a usage message.
 *
 * <p>Note that this is
 * security by obscurity in that the password is not store in plaintext, but it can be recovered by
 * simply using the code from this class.
 *
...
public class FilePassword {
  ...
  static char[] decode(RandomAccessFile passwordFile) throws Exception {
    ...
    int bcount;
    while ((bcount = passwordFile.read()) >= 0) {
      baos.write(bcount);
    }
    passwordFile.close();
  ...
  /**
   * Write a password in opaque form to a file for use with the FilePassword accessor in conjunction
   * with the JaasSecurityDomain {CLASS}org.jboss.security.plugins.FilePassword:password-file format
   * of the KeyStorePass attribute.
   */
  ...
  /**
   * Decode password from file to char array.
   * @return char array of decoded password
  ...
  public char[] toCharArray() throws IOException {
    ...
    try {
      ...
      log.log(Level.INFO, "Failed to decode password file: " + passwordFile, exc);
