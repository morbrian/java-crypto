...
public class CryptoUtil {
  ...
  /**
   * Decrypts the given bytes using AES and the given key. The first 16 bytes must be the
   ...
   *
   * @param encrypted A concatenation of the initialization vector and the cipher text to be
   *    decrypted. The first 16 bytes must be the initialization vector.
   * @param key The key to use for decrypting.
   * @param keySize (_optional_) The size of the provided `key`. If omitted, it's assumed to be 256
   *    bits.
   */
  ...
  /**
   * Creates an AES `Cipher` initialized with the given mode (such as `Cipher.ENCRYPT_MODE`) and
   *    key.
   */
  ...
  /**
   * Creates an AES `Cipher` initialized with the given mode (such as `Cipher.ENCRYPT_MODE`), key,
   *    and parameter spec (such as the initialization vector).
   */
